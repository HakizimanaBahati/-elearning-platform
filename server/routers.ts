...
export const appRouter = router({
  ...
  auth: router({
    ...
    register: publicProcedure
      .input(z.object({ email: z.string().email(), password: z.string().min(6), name: z.string().min(2), role: z.enum(["user", "instructor"]).default("user") }))
      .mutation(async ({ ctx, input }) => {
  ...
  courses: router({
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        if ((ctx.user.role as string) !== "instructor" && ctx.user.role !== "admin") {
          throw new TRPCError({ code: "FORBIDDEN" });
        }
  ...
  enrollments: router({
    ...
    enroll: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const existing = await db.getStudentEnrollmentForCourse(ctx.user.id, input.courseId);
  ...
  progress: router({
    record: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const existing = await db.getLessonProgress(input.enrollmentId, input.lessonId);
        if (existing) {
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          ...
          if (userQuery.includes("what") || userQuery.includes("explain")) {
            assistantMessage = "That's a great question! Based on the course content, I can help explain this concept. Could you provide more details about what specifically you'd like to understand?";
          } else if (userQuery.includes("how")) {
            assistantMessage = "I can help you with that! The course materials cover this topic. Would you like me to break down the steps or explain a specific part?";
          } else if (userQuery.includes("tip") || userQuery.includes("advice")) {
            assistantMessage = "Here's a helpful tip for learning this material: Try to relate the concepts to real-world examples. This helps with retention and understanding. What aspect would you like to focus on?";
          } else if (userQuery.includes("practice") || userQuery.includes("exercise")) {
            assistantMessage = "Great idea! Practicing what you've learned is crucial. Try working through the exercises in the course materials. If you get stuck, feel free to ask me specific questions about the problems.";
          } else if (userQuery.includes("stuck") || userQuery.includes("confused") || userQuery.includes("help")) {
            assistantMessage = "Don't worry, you're not alone! Learning new concepts takes time. Let's break it down together. What specific part are you struggling with?";
          } else {
            assistantMessage = "I'm here to help with your learning! Feel free to ask questions about the course content, and I'll do my best to provide clear explanations and helpful guidance.";
          }
          ...
          const errorMessage = "I encountered an issue processing your question. Please try again.";
